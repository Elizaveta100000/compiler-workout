...
let rec compile_expr expr = match expr with
        ...
        | Language.Expr.Binop (operator, left, right) -> (compile_expr left)@(compile_expr right)@[BINOP operator];;
 ...
 let rec compile statement = match statement with
       ...
       | Language.Stmt.Seq (left_s, right_s) -> (compile left_s)@(compile right_s);;
