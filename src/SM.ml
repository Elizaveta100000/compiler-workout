...
let rec eval env (cst, st, (s, input, output)) prg = 
  ...
  | BINOP op :: p -> 
    let y :: x :: st1 = st in 
    let res = Expr.eval s (Binop (op, Const x, Const y)) in 
      eval env (cst, res :: st1, (s, input, output)) p 
...
let label = object
 ...
 let rec compileWithLabels p lastL =
  ...
  in match p with
  | Stmt.Seq (s1, s2)  -> (let newLabel = labelGen#getLabel in
  ...
  | Stmt.If (e, s1, s2) ->
    let lElse = labelGen#getLabel in
  ...
  | Stmt.While (e, body) ->
    let lCheck = labelGen#getLabel in
    let lLoop = labelGen#getLabel in
